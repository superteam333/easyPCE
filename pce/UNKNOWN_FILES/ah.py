...
def favorites(request):
	...
	c = Context({'favorites':favorites, 'zip':zipped, 'user':user})
	return HttpResponse(t.render(c))
...
def index(request):
	...
	c = Context({'depts':depts, 'user':user})
	...
	if 'q' in request.GET and request.GET['q']:
		...
		if re.search('^\".*\"', q) is not None:
			...
			if re.search("^[a-zA-Z]{2,3}", after) is not None:
				#try:
				#	courses = Course.objects.filter(da=after)
				#	if courses is not None:
				#		a = []
				#		for i in range(0, len(courses)):
				#			if i == 0:
				#				courseList.append(courses[i])
				#			advice = Advice.objects.filter(instance=courses[i], text__icontains=text).exclude(text__icontaints("not"))
				...
				#		else:
				#			courseList.pop	
				# Add search for distribution area when available
				try:
					...
					if cnums is not None:
						for n in cnums:
							...
							if instances is None:
								instances = Course.objects.filter(da=after)
						...
						html = t.render(Context({'results':res,'query':q}))
		...
		if (czipped or pzipped or dzipped):
			...
			html = t.render(Context({'courses':czipped, 'profs':pzipped, 'depts':dzipped, 'query':q}))
			return HttpResponse(html)			
