...
# * need a try here?? and everywhere netid is needed? *
	if request.session['netid'] is None:
	...
	if favorites is not None:
		for i in range(len(favorites)):
			...
			try:
				...
				nextSemester.append(0) 
	t = get_template("dev/myfavorites.html")
...
def search(request):
	...
	if 'q' in request.GET and request.GET['q']:
			...
			if re.search("^[a-zA-Z]{2,3}", after) is not None:
				...
				try:
					...
					if instances is not None and len(instances) > 0:
						...
						t = get_template('dev/textsearch.html')
		...
		if (czipped or pzipped or dzipped):
			t = get_template('dev/search_results.html')
		...
		t = get_template('dev/search_empty.html')
